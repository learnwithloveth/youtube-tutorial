...
import { seededRandom, hashSeed } from '@/features/markets/simulation';
...
/** 90 deterministic daily bars — stable across renders and screenshots. */
function UptimeBars({ seed, health }: { seed: string; health: Health }) {
  const rand = seededRandom(hashSeed(seed));
  const bars = Array.from({ length: 90 }, (_, i) => {
    const roll = rand();
    if (i > 86 && health !== 'operational') return health === 'degraded' ? 'warn' : 'accent';
    if (roll > 0.985) return 'warn';
    return 'up';
  });
  return (
...
export default function StatusPage() {
  useSeo({
    title: 'System status',
    description:
      'Live status for the Novex matching engine, APIs, apps and payment rails, with a 90-day uptime history and full incident log.',
  });
      ...
      <PageHero
        ...
        actions={
          <>
            ...
            <ButtonLink to="/blog" variant="ghost" size="lg">
