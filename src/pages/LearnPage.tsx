...
import { LEARN_TRACKS } from '@/data/content';
...
export default function LearnPage() {
  useSeo({
    title: 'Novex Academy',
    description:
      'Twelve lessons across three tracks — foundations, trading mechanics and beyond spot. Written to be useful, not to make you trade more.',
  });

  const totalMinutes = LEARN_TRACKS.flatMap((t) => t.lessons).reduce((sum, l) => sum + l.minutes, 0);
  ...
  return (
    <>
      <PageHero
        ...
        actions={
          <>
            <ButtonLink to="#tracks" size="lg" sheen>
      ...
      <Section id="tracks" tone="sunken">
        <div className="shell space-y-20">
          {LEARN_TRACKS.map((track, trackIndex) => (
            <div key={track.name}>
              <Reveal>
      ...
      <CtaBand
        ...
        secondary={{ label: 'See the fees first', to: '/fees' }}
      />
    </>
  );
}
