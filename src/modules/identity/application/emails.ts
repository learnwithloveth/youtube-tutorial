...
function layout(heading: string, body: string, action: { label: string; url: string }): string {
  return `<!doctype html>
<html lang="en">
  <body style="margin:0;padding:24px;background:#f4f4f7;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#1a1a1a;">
    <div style="max-width:520px;margin:0 auto;background:#ffffff;border-radius:12px;padding:32px;">
      <p style="margin:0 0 24px;font-size:18px;font-weight:600;letter-spacing:-0.02em;">${BRAND_NAME}</p>
      <h1 style="margin:0 0 16px;font-size:22px;font-weight:600;line-height:1.3;">${heading}</h1>
      <p style="margin:0 0 24px;font-size:15px;line-height:1.6;color:#4a4a55;">${body}</p>
      <a href="${action.url}" style="display:inline-block;background:#6d28d9;color:#ffffff;text-decoration:none;padding:12px 24px;border-radius:999px;font-size:15px;font-weight:500;">${action.label}</a>
      <p style="margin:24px 0 0;font-size:13px;line-height:1.6;color:#77778a;">
        If the button does not work, paste this into your browser:<br />
        <span style="word-break:break-all;color:#6d28d9;">${action.url}</span>
      </p>
      <hr style="border:none;border-top:1px solid #e6e6ee;margin:28px 0 16px;" />
      <p style="margin:0;font-size:12px;line-height:1.6;color:#8f8fa3;">
        ${BRAND_NAME} is a fictional exchange built for design and demonstration.
...
export function renderVerificationEmail(input: {
  ...
  return {
    ...
    text: [
      ...
      ``,
      `Open this link to confirm the address on your ${BRAND_NAME} account:`,
      input.url,
      ``,
      `The link is valid for ${validFor} and can be used once.`,
      `If you did not create an account, you can ignore this message.`,
      ``,
      `${BRAND_NAME} will never ask you for your password or a verification code by email.`,
    ].join('\n'),
    html: layout(
      'Confirm your email address',
      `Open the link below to confirm the address on your ${BRAND_NAME} account. It is valid for ${validFor} and can be used once. If you did not create an account, you can ignore this message.`,
