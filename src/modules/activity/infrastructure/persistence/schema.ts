 ...
 *
 * Module-local, prefixed `ac_`. Append-only: nothing in this module issues an
 * UPDATE, and the only DELETE is the retention sweep. An audit row that can be
 * edited is not an audit row.
 *
 * ── No foreign key to id_users, for the reason presence has none ───────────────
 * `user_id` holds an opaque `UserId`. Identity owns `id_users`, and the rule that
 * keeps identity extractable is that no other context reads its tables — a foreign
 * key is a read the database enforces on every write. The cost is an unenforced
...
export const events = pgTable(
  ...
  (table) => [
    ...
    index('ac_events_user_idx').on(table.userId, table.occurredAt),
    ...
    index('ac_events_user_kind_idx').on(table.userId, table.kind, table.occurredAt),
    ...
    index('ac_events_kind_time_idx').on(table.kind, table.occurredAt),
  ],
);
