...
import {
  Bell, Check, Copy, Fingerprint, Key, Laptop, Plus, ShieldCheck, SlidersHorizontal, Trash2, User,
} from 'lucide-react';
            ...
            <>
              ...
              <Panel>
                <PanelHeader
                  ...
                  subtitle="Signing out a session revokes its tokens immediately"
                />
                <ul className="divide-y divide-line/60">
                  {SESSIONS.map((session) => (
                    <li key={session.id} className="flex flex-wrap items-center gap-3 py-3.5">
                      ...
                      {session.current ? (
                        <Badge tone="up">This device</Badge>
                      ) : (
                        <button
                          type="button"
                          className="rounded-sm border border-line px-2.5 py-1 text-2xs text-fg-muted transition-colors hover:border-down/50 hover:text-down"
                        >
                          Sign out
                        </button>
                      )}
