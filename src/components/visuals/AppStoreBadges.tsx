...
function Badge({
  ...
  return (
    ...
    >
      <svg viewBox="0 0 24 24" className={compact ? 'size-5' : 'size-7'} fill="currentColor" aria-hidden>
        <path d={path} />
      </svg>
...
export function AppStoreBadges({
  ...
  return (
    <div className={cn('flex flex-wrap items-center gap-3', className)}>
      <Badge store="App Store" line="Download on the" path={APPLE} compact={compact} />
      <Badge store="Google Play" line="Get it on" path={PLAY} compact={compact} />
    </div>
  );
}
